...
class SphereNearestNeighbour implements Iterable<SphereInstance> {
	...
	public void addNeighbour(SphereInstance otherSphere) {
		if (otherSphere == sphere) return;
		
		double distance = Math.pow(otherSphere.x - sphere.x, 2) + Math.pow(otherSphere.y - sphere.y, 2) + Math.pow(otherSphere.z - sphere.z,  2);
	...
	public Iterator<SphereInstance> iterator() {
		Iterator<SphereInstance> iterator = new Iterator<SphereInstance>() {
			private Iterator<KeyValuePair<SphereInstance, Double>> internalIterator = distances.iterator();
